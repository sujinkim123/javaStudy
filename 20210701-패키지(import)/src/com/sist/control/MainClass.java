 ...
 *                  public 선언 : 생성자(클래스를 읽어 올 때 메모리에 저장을 한다 => new 생성자())
 *                                => public 클래스명()
 *                                메소드는 다른 클래스와 통신을 할 때 사용
 *                                => public 리턴형 메소드명(매개변수...)
 *                                클래스 
 *                                => public class 클래스명
 *         protected => 패키지 안에서 접근이 가능 + 상속 받은 클래스(다른 패키지) => 거의 사용빈도가 없다
 *         default => 같은 패키지 안에서
 *         private => 클래스 내에서 (자신의 클래스) : 변수 => 은닉화
 *                                                    ==== 읽기메소드/쓰기메소드 첨부 => 캡슐화
 *                                                         =====================
 *                                                         getter/setter
 *     2. 기타제어자
 ...
 *                      static int a;
 *                      public void display() {
 *                          static int b; ==> 오류 (지역변수는 static을 사용할 수 없다)
 *                      }
 *                  }
 *        ====== 변수 , 메소드 , 클래스 (내부클래스)
