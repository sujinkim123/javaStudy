...
public class 배열활용_문자열 {
	...
	public static void main(String[] args) throws Exception {
		...
		int index = 0;
		for (int i = 1; i <= 4; i++) {
			Document doc = Jsoup.connect("https://www.genie.co.kr/chart/top200?ditc=D&ymd=20210621&hh=15&rtm=Y&pg=" + i)
					.get();
			Elements titles = doc.select("a.title");
			Elements singers = doc.select("a.artist");
			Elements albums = doc.select("a.albumtitle");
			for (int j = 0; j < 50; j++) {
				/* System.out.println((index + 1) + "." + titles.get(j).text() + " " + singers.get(j).text() + " "
						+ albums.get(j).text());
				*/
		...
		*/
		...
		String data2 = stdIn.next();
		int res = 0;
		for (int k=0; k<singer.length; k++) {
			if (singer[k].startsWith(data2)) {	// equals , startsWith , contains
