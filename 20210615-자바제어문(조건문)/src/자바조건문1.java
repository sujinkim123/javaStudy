 ...
 *        => 로그인 , 아이디 중복체크 , 우편번호(입력이 안된 경우, 없는 데이터)
 *        
 *      형식)
 *             if(조건문) {
 *                 // 조건이 true일 때 처리하는 문장 => 아이디가 중복이 안된 경우
 *             }
 *             else {
 *                 // 조건이 false일 때 처리하는 문장 => 아이디가 중복이 되었다면
 *             }
 *             
...
public class 자바조건문1 {
	public static void main(String[] args) {
		...
		// 1. Scanner => 데이터형 (클래스: 사용자 정의) 메모리에 저장해서 사용 => 저장할 때 new
		// 생성 => 클래스명  변수명 = new 클래스명() ==> 클래스명() 생성자함수 => 클래스가 가지고 있는 변수 초기화
		/*
		 * 		클래스의 구성 요소
		 *        1. 변수	2. 메소드	 3. 생성자  ==> 객체지향 프로그램
		 */
		Scanner stdIn = new Scanner(System.in);	// System.in(키보드 입력값을 받는 경우) , System.out(화면 출력)
		/*
		 *		사용자 입력
		 *        정수 ==========> nextInt()
		 *        문자열 ========> next()
		...
		// 저장
		int num = stdIn.nextInt();	// num 안에 사용자 입력한 값이 저장
		if (num%2==0) {	// 짝수 조건
