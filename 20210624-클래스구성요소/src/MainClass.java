 ...
 *  구성요소
 *   변수 (멤버변수)
 *       = 인스턴스 변수 => 저장공간 따로 생성 new (저장공간 별도 생성) : String
 *         A a = new A() => 저장공간 따로 생성
 *         B b = new B() => 저장공간 따로 생성
...
class Student {
	String name;
	int kor,eng,math; // 인스턴스 변수 => new라는 연산자를 이용해서 저장공간을 만든 다음에 저장
...
public class MainClass {
	public static void main(String[] args) {
		...
		// 이름 저장 2개
		String name1, name2, namei;
		int kor1, kor2, kori;
		int eng1, eng2, engi;
		int math1, math2, mathi;
		...
		System.out.printf("%5s%4d%4d%4d%5d%7.2f\n", name2, kor2, eng1, math2, (kor2+eng2+math2), (kor2+eng2+math2)/3.0);
		System.out.printf("%5s%4d%4d%4d%5d%7.2f\n", namei, kori, eng1, mathi, (kori+engi+mathi), (kori+engi+mathi)/3.0);
		...
		for (int i=0; i<name.length;i++) {
			System.out.printf("%5s%4d%4d%4d%5d%7.2f\n", name[i], kor[i], eng[1], math[i], (kor[i]+eng[i]+math[i]), (kor[i]+eng[i]+math[i])/3.0);
		}
		/*  String[] names;
		 *  int[] kor;
		 *  int[] eng;
		 *  int[] math;
		 *  ==================> index가 같은 것이 학생 한명에 대한 정보
		 */
