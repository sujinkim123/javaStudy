...
public class 변수초기화2 {
	...
	// 1. 생성자 이용 => 명시적 초기화가 안되는 부분에서 생성자가 초기화
	// 생성자 => 멤버변수 => 초기화
	// 생성자 => 오버로딩을 지원한다 (생성자 1개가 아니라 여러개를 만들 수 있다)
	// 사용자로부터 입력을 받아서 처리 => 매개변수
	/*
	 *   초기화 : 직접 값을 입력해서 메모리 저장 (클래스에서 값을 지정) ex) int = 10;
	 *   사용자 : 입력값을 받아서 메모리 저장 => 생성자
	 *   컴퓨터 난수 메모리 저장 => 생성자
	 *   
	 *   생성자는 클래스를 메모리에 저장할 때 처음으로 호출되는 메소드
	 *   ***** 생성자 호출시에
	 *   class A {
	 *       A() {} => 클래스명과 동일 , 리턴형이 없다
	 *       void A() {} => 일반 메소드
