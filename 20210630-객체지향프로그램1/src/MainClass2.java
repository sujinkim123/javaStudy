/*
 *   객체지향답게 프로그램 제작
 *   1) 데이터 보호 (다른 클래스에서 직접 변수 사용을 할 수 없다, 단 사용이 가능하게 만든다(메소드를 이용해서))
 *      =========== 은닉화 / 캡슐화
 *        === 접근지정어
 *            private
 *            protected
 *            public 
 *            default
 *   2) 기존에 만든 클래스를 재사용 (상속=변경해서 사용, 포함=있는그대로)
 *                                   =================== ================
 *                                       사용자 정의        라이브러리
 *                                   ==================================== extends(확장) => 형변환
 *   3) 변경, 추가 (다형성)
 ...
 *              기능 : 읽기              /             쓰기                     
 *                     ====                            ====
 *                     메모리에 값을 가지고 온다       메모리에 저장 => 메소드 읽기:getter , 쓰기:setter ==> 라이브러리(lombok)
 *       ***** 메소드는 다른 클래스 통신 public
 *       ***** 생성자 => 어떤 클래스든 상관없이 사용이 가능 public
 *   
 */
/*
 * java 파일 안에 여러개 class를 제작
 * =========
 *  = public Class => 1번만 사용이 가능
 *  = 저장명은 public class인 명으로 저장
 * 
 *  class A
