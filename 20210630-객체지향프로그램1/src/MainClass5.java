/*
 *  310 page
 *   => 클래스의종류, 예외처리 => 라이브러리 => 응용
 *   => 상속 (소스 간결하다, 코드를 변경, 관리가 편리, 코드의 중복을 제거)
 *            =============                            ==================
 *   => 단점 (가독성X, 최적화X)
 *   class A
 *   class B extends A ==> 1) A를 메모리 할당
 *                         2) B를 메모리 할당
 *                         3) 여러개의 상속을 받는 경우 소스가 복잡할 수도 있다
 ...
 *    2) 메모리 크기는 상속 받은 클래스가 많다 , 실제 클래스 상속 내린 클래스가 크다
 *    
 *      동물 (동물)
 *        |
 *      인간 (인간, 동물)
 *    
 *    3) static은 상속을 내리는 것이 아니고 사용이 가능하게 만들어 준다
 *    4) private은 상속은 내리지만 접근이 불가능
 *       => 생성자, 초기화블록, static, private 상속 예외
 *    5) 상속이 있는 경우 위로 올라갈수록 크기가 커진다
