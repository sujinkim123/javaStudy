 ...
 *  3) 메소드 안에는 재사용을 목적으로 하기 때문에 한 개의 기능만 수행이 가능하게 한다
 *     예) 찾기 , 목록 , 데이터 추가 .....
 *         댓글 (댓글 쓰기, 댓글 보기, 댓글 삭제, 댓글 수정 ....)
 *  4) 메소드 제작 방법
 *        리턴형   매개변수
 ...
 *     =    X         X ========> 사용빈도가 낮다
 *  5) 메소드 형식
 *     (접근지정어) 리턴형 메소드(매개변수...) ==> 매개변수는 없을 수도 있고 여러개 사용이 가능 , 단 리턴형은 한개만 사용 (리턴형이 없는 경우는 void를 사용한다)
 ...
 *     void display(int a) {
 *         if (a==0)
 *           return; // 사용자가 0을 입력해주면 메소드 종료 ==> 사용자가 입력하지 않았을 경우
 *         else
 *           return;
 *     }
 *     
 ...
 *            결과값을 받는다
 *            
 *           *** return문이 있을 때까지 메소드 {} 안에 있는 모든 내용을 수행
 *           *** 메소드는 반복제거 할 수 있는 기능이 있다
 *           
 *     => class
 *        Tv
 ...
 *           동작 : 먹는 기능 , 걷는 기능 , 뛰는 기능 , 냄새 맡는 기능 ....
 *           ============================================ 시뮬레이션
 */

/*
 *   1. 멤버변수는 멤버메서드 어떤 곳이든 상관없이 사용이 가능
...
class Temp {
	...
	static void ccc() { // 클래스메소드, 클래스변수 (static) => 정적, 공유변수, 공유메소드
		Temp t = new Temp(); // 객체 선언 후에 사용이 가능 => 사용빈도가 극히 드물다
...
class Tv {
	...
	//===============
	  // Tv를 작동 => 동작 (메소드)
	  void setPower() { // 전원 설정 => 실제 동작이 아니라 (가정) => power=true (Tv 켜기) , power=false (Tv 끄기)
		  power =! power;
	  }
	  // channel 이동
	  void channelUp() {
...
public class MainClass {
	public static void main(String[] args) {
		...
		Tv t = new Tv(); // t(메모리 주소:5d22bbb7) => color,power,channel,setPower,channelUp,channelDown
...
//		t2.setPower();
		...
		// => new를 이용하지 않아도 메모리에 저장 => 필요할 때마다 new를 사용하지 않고 사용이 가능
		// Scanner scan = new Scanner() => scan.nextInt() => 인스턴스 메소드
		// Math.random() => static 메소드 (Math=>수학처리 => static)
